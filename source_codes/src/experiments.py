...
from src.emb.complex import ComplEx, Fourier
from src.emb.cnn import ConvE
from src.emb.decom import DistMult, TuckER, LowFER
from src.emb.euclidean import PTransE, TransE

from src.emb.utils import get_conve_kg_state_dict, get_complex_kg_state_dict, get_distmult_kg_state_dict, get_tucker_kg_state_dict
...
def process_data():
    ...
    data_utils.prepare_kb_envrioment(
        raw_kb_path, train_path, dev_path, test_path, args.test, args.add_reverse_relations)
...
def train(lf):
    ...
    if 'NELL' in args.data_dir:
        ...
        seen_entities = data_utils.load_seen_entities(
            adj_list_path, entity_index_path)
    else:
        seen_entities = set()
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        test_metrics = src.eval.hits_and_ranks(test_data, pred_scores, lf.kg.all_objects,
                                               verbose=True, output=False, kg=lf.kg, model_name=args.model, split_relation=False)
        eval_metrics['dev'] = {}
...
def run_ablation_studies(args):
    ...
    for system in systems:
        ...
        if system == '-ad':
            ...
            config_path = os.path.join(
                'configs', '{}.sh'.format(dataset.lower()))
    ...
    print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(
        to_m_ratio, to_m_mrrs['ours'][''], to_m_mrrs['-rs'][''], rel_change(
            to_m_mrrs, '-rs', ''), to_m_mrrs['-ad'][''], rel_change(to_m_mrrs, '-ad', ''),
    ...
    print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(
        seen_ratio, seen_mrrs['ours'][''], seen_mrrs['-rs'][''], rel_change(
            seen_mrrs, '-rs', ''), seen_mrrs['-ad'][''], rel_change(seen_mrrs, '-ad', ''),
    ...
    print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(
        to_m_ratio, to_m_mrrs['ours']['full_kg'], to_m_mrrs['-rs']['full_kg'], rel_change(
            to_m_mrrs, '-rs', 'full_kg'), to_m_mrrs['-ad']['full_kg'], rel_change(to_m_mrrs, '-ad', 'full_kg'),
    ...
    print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(
        seen_ratio, seen_mrrs['ours']['full_kg'], seen_mrrs['-rs']['full_kg'], rel_change(
            seen_mrrs, '-rs', 'full_kg'), seen_mrrs['-ad']['full_kg'], rel_change(seen_mrrs, '-ad', 'full_kg'),
...
def export_error_cases(lf):
    ...
    dev_path = os.path.join(args.data_dir, 'dev.triples')
    dev_data = data_utils.load_triples(
        dev_path, entity_index_path, relation_index_path)
    lf.load_checkpoint(get_checkpoint_path(args))
    print('Dev set performance:')
    pred_scores = lf.forward(dev_data, verbose=False)
    src.eval.hits_and_ranks(dev_data, pred_scores,
                            lf.kg.dev_objects, verbose=True)
    src.eval.export_error_cases(dev_data, pred_scores, lf.kg.dev_objects, os.path.join(
        lf.model_dir, 'error_cases.pkl'))
            ...
            if hasattr(args, arg_name):
                ...
                if type(arg_value2) is str:
                    ...
                    if arg_value == 'True':
                        ...
                        raise ValueError(
                            'Unrecognized boolean value description: {}'.format(arg_value))
                    ...
                    raise ValueError('Unrecognized attribute type: {}: {}'.format(
        ...
        with torch.set_grad_enabled(args.train or args.search_random_seed or args.grid_search):
            if args.search_random_seed:
                ...
                for i in range(num_runs):
                    ...
                    print(
                        'Hits@1 mean: {:.3f}\tstd: {:.6f}'.format(np.mean(hits_at_1s_), np.std(hits_at_1s_)))
                ...
                best_random_seed, best_mrr = sorted(
                    mrrs_search.items(), key=lambda x: x[1], reverse=True)[0]
                ...
                for i, grid_entry in enumerate(list(grid)):
                    ...
                    best_signature, best_mrr = sorted(
                        mrrs.items(), key=lambda x: x[1], reverse=True)[0]
