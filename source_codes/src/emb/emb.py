...
class EmbeddingBasedMethod(LFramework):
            ...
            if decide_num < 25:
                ...
                try:
                    while int(e2[i]) in self.kg.train_objects[int(e1[i])][rand_r]:
                        rand_r = random.randint(
                            0, len(self.kg.relation2id) - 1)
                except:
                    pass
                e1_list.append(int(e1[i]))
                e2_list.append(int(e2[i]))
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        if self.model == 'hypere':
            ...
            pred_scores = mdl.forward(
                e1, r, kg, [self.secondary_kg, self.tertiary_kg])
        else:
            pred_scores = mdl.forward(e1, r, kg)
        return pred_scores
    ...
    def get_subject_mask(self, e1_space, e2, q):
        ...
        for i in range(len(e1_space)):
            ...
            subject_mask = torch.sum(
                e1_space[i].unsqueeze(0) == answer_vector, dim=0)
    ...
    def get_object_mask(self, e2_space, e1, q):
        ...
        for i in range(len(e2_space)):
            ...
            object_mask = torch.sum(
                e2_space[i].unsqueeze(0) == answer_vector, dim=0)
    ...
    def export_reward_shaping_parameters(self):
        ...
        fn_kg_state_dict_path = os.path.join(
            self.model_dir, 'fn_kg_state_dict')
        ...
        print('Knowledge graph embeddings export to {}'.format(
    ...
    def export_fuzzy_facts(self):
        ...
        print('Saving fuzzy facts to {}'.format(
