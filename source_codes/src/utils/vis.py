...
def visualize_step(action_dist, e, action_space, plot):
    ...
    plt_obj = plot.imshow(np.expand_dims(action_dist, 1),
                          interpolation='nearest', cmap=plt.cm.Blues)
    plt.setp(plot, xticks=[0], xticklabels=[e], yticks=range(
        action_space_size), yticklabels=action_space)
    plot.xaxis.tick_top()
...
def visualize_path(query, path_components, output_path=None):
    ...
    gridspec_kwargs = dict(top=0.9, bottom=0.1, left=0.1,
                           right=0.9, wspace=8, hspace=0.4)
