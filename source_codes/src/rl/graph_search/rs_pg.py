...
from src.emb.utils import get_conve_nn_state_dict, get_conve_kg_state_dict, \
...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        if fn_model in ['conve']:
            ...
            fn_state_dict = torch.load(args.lowfer_state_dict_path, map_location=('cuda:' + str(args.gpu)))
            fn_kg_state_dict = get_distmult_kg_state_dict(fn_state_dict)
            ...
            fn_state_dict = torch.load(args.fourier_state_dict_path, map_location=('cuda:' + str(args.gpu)))
            fn_kg_state_dict = get_complex_kg_state_dict(fn_state_dict)
