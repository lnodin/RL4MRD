...
class KnowledgeGraph(nn.Module):
    ...
    def load_graph_data(self, data_dir):
        ...
        self.type2id, self.id2type = load_index(
            os.path.join(data_dir, 'type2id.txt'))
    ...
    def vectorize_action_space(self, data_dir):
        ...
        print("Sanity check: maximum out degree: {}".format(
        ...
        page_rank_scores = load_page_rank_scores(
            os.path.join(data_dir, 'raw.pgrk'))
        ...
        def vectorize_action_space(action_space_list, action_space_size):
            ...
            r_space = torch.zeros(
                bucket_size, action_space_size) + self.dummy_r
            e_space = torch.zeros(
                bucket_size, action_space_size) + self.dummy_e
        ...
        def vectorize_unique_r_space(unique_r_space_list, unique_r_space_size, volatile):
            ...
            unique_r_space = torch.zeros(
                bucket_size, unique_r_space_size) + self.dummy_r
        ...
        if self.args.use_action_space_bucketing:
            ...
            for e1 in range(self.num_entities):
                ...
                self.entity2bucketid[e1, 1] = len(
                    action_space_buckets_discrete[key])
                ...
                for line in f:
                    ...
                    if file_name in ['raw.kb', 'train.triples']:
                        ...
                        if add_reversed_edges:
                            add_subject(e2, e1, self.get_inv_relation_id(
                                r), train_subjects)
                            add_object(e2, e1, self.get_inv_relation_id(
                                r), train_objects)
                    if file_name in ['raw.kb', 'train.triples', 'dev.triples']:
                        ...
                        if add_reversed_edges:
                            ...
                            add_object(
                                e2, e1, self.get_inv_relation_id(r), dev_objects)
                    add_subject(e1, e2, r, all_subjects)
                    add_object(e1, e2, r, all_objects)
                    if add_reversed_edges:
                        add_subject(
                            e2, e1, self.get_inv_relation_id(r), all_subjects)
                        add_object(
                            e2, e1, self.get_inv_relation_id(r), all_objects)
    ...
    def load_fuzzy_facts(self):
        ...
        theta = 0.5
        fuzzy_fact_path = os.path.join(
            self.args.data_dir, 'train.fuzzy.triples')
        count = 0
    ...
    def virtual_step(self, e_set, r):
        ...
        e_space = (r_space.view(batch_size, -1) == r.unsqueeze(1)
                   ).long() * e_space.view(batch_size, -1)
        e_set_out = []
        for i in range(len(e_space)):
            e_set_out_b = var_cuda(np.unique(e_space[i].data))
    ...
    def define_modules(self):
        if not self.args.relation_only:
            ...
            if self.args.model in ['complex', 'rotate', 'fourier']:
