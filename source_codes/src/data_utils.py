    ...
    with open(data_path) as f:
        ...
        for line in f:
            ...
            if seen_entities and (not e1 in seen_entities or not e2 in seen_entities):
                num_skipped += 1
                if verbose:
                    print('Skip triple ({}) with unseen entity: {}'.format(
...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    ...
    relation2id, id2relation = load_index(
...
def get_relations_by_type(data_dir, relation_index_path):
    ...
    print('to-M relations: {}/{} ({})'.format(num_to_M,
          num_rels, num_to_M / num_rels))
    print('to-1 relations: {}/{} ({})'.format(num_to_1,
          num_rels, num_to_1 / num_rels))
