        ...
        if split_relation:
            ...
            for i in range(kg.num_relations):
                if example_length[i] == 0:
                    continue
                split_score[kg.id2relation[i]] = float(
                    hits_at_10_list[i]) / example_length[i]
            l = sorted(split_score.items(), key=lambda x: x[1], reverse=True)
            for term in l:
                print('{}\t{}\t{}'.format(
                    term[0], term[1], example_length[kg.relation2id[term[0]]]))
...
def export_error_cases(examples, scores, all_answers, output_path):
    ...
    print('{}/{} top-1 error cases written to {}'.format(len(top_1_errors),
          len(examples), output_path))
    print('{}/{} top-10 error cases written to {}'.format(len(top_10_errors),
          len(examples), output_path))
