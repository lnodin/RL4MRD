...
def compute_venn_areas(model_error_list):
    def intersect(m_e1, m_e2):
        if m_e1.name:
            inter_model_errors = ModelErrors(
                '{} & {}'.format(m_e1.name, m_e2.name))
    ...
    all_top_1_errors = set(
        [e for m_e in model_error_list for e in m_e.top_1_error_cases])
    all_top_10_errors = set(
        [e for m_e in model_error_list for e in m_e.top_10_error_cases])
    ...
    for i in sorted(subset_overlap.keys()):
        print('|{}|: {}'.format(subset_overlap[i].name, len(
            subset_overlap[i].top_1_error_cases)))
    print()
    print('Top 10 Error Cases')
    for i in sorted(subset_overlap.keys()):
        print('|{}|: {}'.format(subset_overlap[i].name, len(
            subset_overlap[i].top_10_error_cases)))
